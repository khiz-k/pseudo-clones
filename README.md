...
MERN TikTok (pause/play, snap functionality, IDENTICAL ui): https://tiktok-clone-8ade4.web.app/ \* Tiktok has permission issues where it disallows its' videos from being shared on other sites after certain periods of time. As a result, I didn't want to have to delete database docs and add new ones everyday. To run locally: you need to configure your own mongodb backend or you can use it in conjunction with a firebase database, either config is pretty easy to setup.
